...
def getInfo(msg_received):
    ...
    try:
        ...
        cursor.execute("SELECT * FROM user where (phoneNo = " + "'" + email + "' and password = " + "'" + pass_word + "') OR (email = " + "'" + email + "' and password = " + "'" + pass_word + "') OR (userName = "+"'" + email+"' and password = "+"'" + pass_word + "')")
        row = cursor.fetchall()
        ...
        if len(row) == 1:
            ...
            if len(verified) == 1:
                for record in verified:
                    d['verified'] = record[4]
                    d['token'] = tokens.generate_token(id_no).decode("utf-8")
